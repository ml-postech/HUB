...
class Conditional_Model(nn.Module):
    def __init__(self, config):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                if i_level == 3:
                    # up.upsample = nn.Sequential([
                    #     Upsample(block_in, resamp_with_conv),
                    #     nn.ZeroPad2d(1)
                    # ])
    ...
    def forward(self, x, t, c, mode, cond=False, **kwargs):
        ...
        if mode == "train":
            ...
            if not cond:  # unconditional generation
                return self._forward_with_uncond(x, t, c)
            else:
                cond_scale = kwargs.get("cond_scale")
                return self._forward_with_cond_scale(x, t, c, cond_scale=cond_scale)

    def _forward_with_uncond(self, x, t, c):
        null_logits = self._forward(x, t, c, cond_drop_prob=1.0)
        return null_logits

    def _forward_with_cond_scale(self, x, t, c, cond_scale=1.0):
        null_logits = self._forward(x, t, c, cond_drop_prob=1.0)
    ...
    def _forward(self, x, t, c, cond_drop_prob=None):
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                if h.shape[-1] == 6:  # Changes
                    h = F.pad(h, (0, 1, 0, 1))  # Changes
