...
from source.bias.config import timesteps, sample_type, skip_type, eta, cond_scale
...
def sample_image(model, betas, x, c, use_cond=False, last=True):
    ...
    if sample_type == "generalized":
        ...
        if skip_type == "uniform":
            ...
            raise NotImplementedError
        x = ddpm_step_conditional(x, c, seq, model, betas, cond_scale)
    else:
        raise NotImplementedError
    if last:
        x = x[0][-1]
...
def generalized_steps(x, seq, model, b):
    with torch.no_grad():
        n = x.size(0)
...
def generalized_steps_conditional(x, c, seq, model, b, cond_scale=3.0, **kwargs):
    with torch.no_grad():
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            xt = xs[-1].to(x.device)
            et = model(
                xt, t, c, cond_scale=cond_scale, cond=kwargs.get("cond"), mode="test"
            )
            x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()
            x0_preds.append(x0_t.to("cpu"))
            c1 = eta * ((1 - at / at_next) * (1 - at_next) / (1 - at)).sqrt()
...
def ddpm_step_conditional(x, c, seq, model, b, cond_scale):
    with torch.no_grad():
        n = x.size(0)
        seq_next = [-1] + list(seq[:-1])
        xs = [x]
