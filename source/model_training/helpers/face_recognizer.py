...
from source.model_training.helpers.clustering import clusterize
from source.model_training.helpers.resnet_model import ResNetModel, variable_to_numpy
...
class FaceRecognizer(object):
    def __init__(self, labels, resources_path, top_n=5, use_cuda=False):
        ...
        self.model = ResNetModel(
            len(self.labels.labels_list), weights_path=resources_path, use_cuda=use_cuda
        )
    ...
    def _calculate_predictions(self, face_images):
        predictions_tensor, embeddings_tensor = self.model(
            face_images, return_tensors=True
        )
        ...
        centers = variable_to_numpy(
            self.model.model_with_loss.center_loss_criterion.centers[classes.data]
        )
        similarities = np.array(
            [
                1
                - self._distance(
                    centers[i].reshape(1, -1), embeddings[i].reshape(1, -1)
                ).squeeze()
        ...
        for cluster in clusters:
            ...
            if np.any(cluster_prediction > 0.0):
                ...
                results.append(
                    (
                        [
                            (self.labels.by_model_id[i], cluster_prediction[i])
