...
from source.model_training.models.cnn.resnet import AdaptResNetBottleneck, ResNetCenterLoss
...
class ResNetModel(object):
    ...
    def build_resnet_50(self):
        ...
        model_with_bottleneck = AdaptResNetBottleneck(
            model, self.embedding_size, self.num_classes
        )
        ...
        model_with_loss = (
            model_with_loss.cuda() if self.use_cuda else model_with_loss.cpu()
        )
    ...
    def __call__(self, crops_numpy_array, return_tensors=False):
        ...
        softmax = nn.functional.softmax(fc2_output, dim=1)
    ...
    def _numpy_crops_as_tensor(self, crops):
        with torch.no_grad():
            return Variable(
                self._cast(torch.stack([ToTensor()(crop) for crop in crops]))
            )
